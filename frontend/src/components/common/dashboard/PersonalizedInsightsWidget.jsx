...
export default function PersonalizedInsightsWidget({ reportId }) {
  ...
  return <GlassCard style={{ padding: "22px 24px" }} aria-label={t("dashboard:personalizedInsights", "Personalized insights")}>
    <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
      <div><h3 style={{ margin: 0, fontFamily: "Cinzel,serif", color: "var(--nv-text-primary, #e8d5ff)" }}>{t("dashboard:yourPersonalizedGuidance", "✦ Your personalized guidance")}</h3><p style={{ margin: "6px 0 0", color: "var(--nv-text-muted)", fontSize: 13 }}>{data?.summary?.body || "Loading guidance from your latest saved reading…"}</p></div>
      <div role="group" aria-label={t("dashboard:insightPeriod", "Insight period")}>{["daily", "weekly", "monthly"].map((item) => <button key={item} type="button" onClick={() => setPeriod(item)} style={{ marginLeft: 6, border: 0, borderRadius: 14, padding: "6px 10px", cursor: "pointer", background: period === item ? "rgba(191,127,255,.35)" : "transparent", color: "var(--nv-text-primary)" }}>{item}</button>)}</div>
    </div>
    ...
    {data?.whatsChanged?.available && <p style={{ margin: "14px 0 0", fontSize: 13, color: "var(--nv-text-muted)" }}><strong>{t("dashboard:whatsChanged", "What’s changed:")}</strong> {data.whatsChanged.message}</p>}
  </GlassCard>;
}
