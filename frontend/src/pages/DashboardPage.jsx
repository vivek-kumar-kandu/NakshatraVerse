...
=======
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 14 }}>
            ...
            <StatCard icon="🗓️" label={t("dashboard:statistics.memberSince")} value={formatDate(user?.createdAt, t, language)} accent="#9dc9ff" />
            <StatCard icon="🕐" label={t("dashboard:statistics.latestReading")} value={recentReports[0] ? formatDate(recentReports[0].createdAt, t, language) : t("dashboard:statistics.notAvailable")} accent="#ffd700" />
            <StatCard
              icon={resolvedTheme === "light" ? "☀️" : "🌙"}
...
=======
            ...
            <SummaryCard icon="✨" title={t("dashboard:astrologySummary.cardTitle")}>
              {astroSummary.nakshatra && (
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 12 }}>
                  <Badge color="#ffd700">{astroSummary.nakshatra}{astroSummary.pada ? t("dashboard:astrologySummary.padaBadge", { pada: astroSummary.pada }) : ""}</Badge>
                  {astroSummary.lord && <Badge color="#bf7fff">{t("dashboard:astrologySummary.lordBadge", { lord: astroSummary.lord })}</Badge>}
                </div>
              )}
...
=======
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(170px, 1fr))", gap: 14 }}>
            ...
            {lastOpenedReportId && (
              <QuickActionCard icon="↩️" label={t("dashboard:quickActions.continueReading.label")} desc={t("dashboard:quickActions.continueReading.desc")} onClick={() => onViewReport(lastOpenedReportId)} />
            )}
...
=======
              ...
              onClick={() => (latestFullReport ? onOpenLifeCoach?.(latestFullReport) : onNavigate("ai-life-coach"))}
            />
            <QuickActionCard icon="💞" label={t("dashboard:quickActions.kundliMatching.label")} desc={t("dashboard:quickActions.kundliMatching.desc")} onClick={() => onNavigate("matching")} />
            <QuickActionCard icon="🕉️" label={t("dashboard:quickActions.panchangMuhurat.label")} desc={t("dashboard:quickActions.panchangMuhurat.desc")} onClick={() => onNavigate("panchang")} />
